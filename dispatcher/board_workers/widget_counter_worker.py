...
async def widget_counter_worker(raw_event):
...
async def widget_counter_subscription():
    """Subscription to the widget counter event
    
        Creates a filtered subscription on properties monitored by the widget counters provisioned in the system.
        
        When an event is received on the subscription channel it is passed to widget_counter_worker() for processing.
    ...
    try:
        ...
        transport = AIOHTTPTransport(url="http://{}/graphql".format(gql_address), headers={'Authorization': 'Bearer ' + jwt})
        client = Client(transport=transport, fetch_schema_from_transport=False)
        # Find all the board widget counters
        find_widgets_query = gql(
            """
            query FindWidgets1 {
                objects(filter: {
        ...
        transport = WebsocketsTransport(url="ws://{}/graphql".format(gql_address), headers={'Authorization': 'Bearer ' + jwt}, keep_alive_timeout=30)
        async with Client(
